...
'''
8 Numero medio di volte in cui un cliente prenota in un anno in un area geolocalizzata
'''


def get_geo_area_average_users_books_in_date_range(lat, long, km_radius, date_start, date_end):
    db = get_db()

    max_lat, max_long = sum_km_to_coordinates(lat, long, km_radius)
    min_lat, min_long = sum_km_to_coordinates(lat, long, -km_radius)

    query = "SELECT AVG(reservation) AS average_reservation " \
            "FROM ( " \
    ...
    return result[0]['average_reservation'] if result[0]['average_reservation'] else 0
